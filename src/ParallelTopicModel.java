...
public class ParallelTopicModel implements Serializable {
	...
	public ArrayList<TreeSet<IDSorter>> getSortedWords () {
		...
		for (int type = 0; type < numTypes; type++) {
            ...
            //@Arun - to add weights dynamically
			int index = 0;
            String word = alphabet.lookupObject(type).toString();
            int modifiedWeight=(int) Math.pow(3,word.split("_").length-1);
            // ends here
			while (index < topicCounts.length &&
                ...
                //@Arun - to add the weights dynamically
                count= count*modifiedWeight;
                //ends here
